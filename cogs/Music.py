...
class Source:
    ...
    @staticmethod
    def from_youtube_id(video_id: str) -> Source:
        address = f"https://youtube.com/watch?v={video_id}"
        pyt = pafy.new(address)
        return Source(address=address, source_type=SourceType.youtube, name=f"{pyt.title} ({pyt.duration})")
    @staticmethod
    def from_address(address: str) -> Source:
        ...
        if address[0:2] == './': # Local file
            return Source(address[2::], SourceType.local, address[2::])
        elif 'youtube.com/watch?v=' in address: # YouTube video
            video_id = re.findall(r"watch\?v=(\S{11})", address)[0]
            return Source.from_youtube_id(video_id)
        else:
            #Search YouTube for string
            query = address.replace(' ', '+')
            html = urllib.request.urlopen("https://www.youtube.com/results?search_query=" + query)
            video_ids = re.findall(r"watch\?v=(\S{11})", html.read().decode())
            for i in range(0, 5):
                return Source.from_youtube_id(video_ids[0])
...
class Music(commands.Cog):
    ...
    async def play(self, ctx: commands.Context):
        ...
        await ctx.send(f"Added to queue: {source.name}")
